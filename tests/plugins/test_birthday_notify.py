...
def reset_globals(monkeypatch):
    # Reset all global state between tests, including JOINED_ROOMS and
    # ANNOUNCED_TODAY
...
def bot(monkeypatch):
    ...
    monkeypatch.setattr(
        birthday_notify, "handle_room_toggle_command",
        handle_room_toggle_command)
    # Patch _is_enabled_for_room to True
    async def always_enabled(*a, **k): return True
    monkeypatch.setattr(
        birthday_notify, "_is_enabled_for_room", always_enabled)
...
def test_normalize_bday_value():
    ...
    assert birthday_notify._normalize_bday_value(
        ["", "1991-02-03"]) == "1991-02-03"
...
def test_calculate_age(monkeypatch):
    ...
    assert birthday_notify._calculate_age(
        f"{today.month:02}-{today.day:02}") is None
...
async def test_check_user_birthday_announce(monkeypatch, bot):
    ...
    monkeypatch.setattr(
        birthday_notify, "_get_birthday_cached_or_live",
        _get_birthday_cached_or_live)
    # So is_birthday_today returns True, path with age: triggers announcement
    await birthday_notify._check_user_birthday(bot, "user@x", "Nick",
                                               "room@room")
...
async def test_check_user_birthday_exception(monkeypatch, bot, caplog):
    ...
    monkeypatch.setattr(
        birthday_notify, "_get_birthday_cached_or_live", raiseit)
    with caplog.at_level(logging.ERROR):
        await birthday_notify._check_user_birthday(bot, "u@x", "Nick", "r@r")
    assert any(
        "Error checking user birthday:" in r.message for r in caplog.records)
...
async def test_birthday_notify_command_enabled_checks_room(monkeypatch, bot):
    ...
    monkeypatch.setattr(
        birthday_notify, "handle_room_toggle_command", dummy_handle)
    ...
    await birthday_notify.birthday_notify_command(bot, "s", "n", ["on"],
                                                  msg, False)
...
async def test_get_birthday_from_vcard(monkeypatch, bot):
    ...
    out = await birthday_notify._get_birthday_from_vcard(bot, "room@conf",
                                                         "Nick")
...
async def test_get_birthday_cached_or_live(monkeypatch, bot):
    ...
    result = await birthday_notify._get_birthday_cached_or_live(bot,
                                                                "room@conf",
                                                                "jid3",
                                                                "Nick3")
    ...
    result2 = await birthday_notify._get_birthday_cached_or_live(bot,
                                                                 "room@conf",
                                                                 "jid3",
                                                                 "Nick3")
    ...
    monkeypatch.setattr(birthday_notify, "_now_ts", lambda: ts)
    result3 = await birthday_notify._get_birthday_cached_or_live(bot,
                                                                 "room@conf",
                                                                 "jid4",
                                                                 "Nick4")
...
async def test_load_and_mark_announced(bot):
    ...
    assert (await birthday_notify._load_announced_date(bot, "roomA", "jidA")
            is None)
