...
def fake_bot():
    ...
    bot.db.users = SimpleNamespace(
        _nick_index={"Nick": {"jidval"}, "ListNick": [
            "jid1", "jid2"], "SingleNick": "jid3"},
        _nick_index_lock=asyncio.Lock(),
        plugin=lambda plugin: SimpleNamespace(
    ...
    bot.bot_plugins.plugins = {"rooms": SimpleNamespace(
        JOINED_ROOMS={"room@conf": {
            "nicks": {
                "Nick": {"jid": "jidval", "affiliation": "owner",
                         "role": "moderator"},
...
def msg(from_jid="user@x/resource", resource=None, type_="chat",
        to_jid="bot@domain"):
    if "/" in from_jid:
...
def test_format_status_helpers():
    ...
    for fun in [_core._format_enabled,
                _core._format_disabled,
                _core._format_already_enabled,
                _core._format_already_disabled]:
...
async def test_is_plugin_enabled_for_room(fake_bot):
    ...
    res = await _core.is_plugin_enabled_for_room(fake_bot, async_store_getter,
                                                 "any", "room@conf")
    assert res
...
async def test_is_room_moderator_or_admin(fake_bot):
    ...
    assert allowed
    allowed2 = await _core.is_room_moderator_or_admin(fake_bot, room_jid,
                                                      "Nonexistent")
    assert not allowed2
...
async def test_handle_room_toggle_command_all_paths(fake_bot, monkeypatch):
    ...
    for subcmd in ["on", "off", "status"]:
        ...
        async def store_getter(b):
            return store

        await _core.handle_room_toggle_command(
            fake_bot, {}, False, [subcmd], store_getter=store_getter,
            key="key", label="lbl", storage="dict"
        )
...
#        async def storeL_getter(b):
#            return storeL
#        await _core.handle_room_toggle_command(
#            fake_bot, {}, False, [subcmd], store_getter=storeL_getter,
#            key="key", label="lbl", storage="list"
#        )
    ...
    result = await _core.handle_room_toggle_command(
        fake_bot, {}, False, ["on"], store_getter=store_getter,
        key="key", label="lbl")
