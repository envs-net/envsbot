...
async def muc_pm_sender_can_manage_room(bot, msg, is_room: bool) -> tuple[bool, str, Optional[str]]:
    ...
    """
    if is_room:
        return False, "", "🔴 This command can only be used in a MUC DM."

    room_jid, nick = _room_and_nick_from_muc_pm(msg)
    if room_jid not in JOINED_ROOMS:
        return False, room_jid, "🔴 This command can only be used in a MUC DM."
...
async def handle_room_toggle_command(
    ...
    *,
    ...
    Returns True when args[0] is one of on/off/status and the command was fully handled.
    Returns False for all other subcommands so the plugin can continue normal handling.

    Supported storage formats:
      - storage="dict": {room_jid: True}
      - storage="list": {list_field: [room_jid, ...]}
    """
    if not args or args[0] not in _CONTROL_COMMANDS:
        return False
    ...
    if storage == "dict":
        ...
        if subcmd == "status":
            bot.reply(msg, f"{'✅' if enabled else '🛑'} {label} are **{'enabled' if enabled else 'disabled'}** in this room.")
            return True

        if subcmd == "on":
            if enabled:
                bot.reply(msg, f"ℹ️ {label} already enabled.")
                return True
            state[room_jid] = True
            await store.set_global(key, state)
            bot.reply(msg, f"✅ {label} enabled in this room.")
            log.info("%s Room %s enabled", log_prefix, room_jid)
            return True

        if not enabled:
            bot.reply(msg, f"ℹ️ {label} already disabled.")
            return True
        state.pop(room_jid, None)
        await store.set_global(key, state)
        bot.reply(msg, f"🛑 {label} disabled in this room.")
    ...
    if storage == "list":
        ...
        if subcmd == "status":
            bot.reply(msg, f"{'✅' if enabled else '🛑'} {label} is **{'enabled' if enabled else 'disabled'}** in this room.")
            return True
