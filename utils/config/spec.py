...
CONFIG_FIELDS: dict[str, ConfigField] = {
    ...
    'runtime_data_dir': ConfigField(None, 'RUNTIME_DATA_DIR', str, startup_only=True, section='Bot Runtime', description='Directory for mutable support files such as vcard.py, chat_slang.csv, slang review queues and profile hash markers. None keeps the historical application-root location for compatibility. Hardened systemd installs should set /var/lib/envsbot.', sample=None),
    ...
    'admin_alert_backup_max_age_hours': ConfigField(36, 'ADMIN_ALERT_BACKUP_MAX_AGE_HOURS', int, minimum=0, minimum_exclusive=True, section='Immediate Admin Alerts', description='Alert when the newest managed backup is older than this many hours.'),
    ...
    'backup_interval_hours': ConfigField(24, 'BACKUP_INTERVAL_HOURS', int, startup_only=True, minimum=0, section='Backups', description='Create an automatic managed backup this many hours after the newest managed backup. Set to 0 to disable periodic backups. This value should be lower than ADMIN_ALERT_BACKUP_MAX_AGE_HOURS so a scheduled backup is created before the stale-backup alert threshold.'),
