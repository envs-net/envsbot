...
CONFIG_FIELDS: dict[str, ConfigKeySpec] = {
    ...
    'message_cache_size': ConfigKeySpec(100, 'MESSAGE_CACHE_SIZE', int, startup_only=True, minimum=0, minimum_exclusive=True, section='Message Cache', description='Number of recent messages retained in RAM per room or private conversation. The same bound applies to SQLite when MESSAGE_CACHE_PERSIST is enabled.'),
    'message_cache_max_age_days': ConfigKeySpec(30, 'MESSAGE_CACHE_MAX_AGE_DAYS', int, startup_only=True, minimum=0, section='Message Cache', description='Remove cached messages older than this many days from RAM and SQLite. Set 0 to disable age pruning.'),
    'message_cache_persist': ConfigKeySpec(True, 'MESSAGE_CACHE_PERSIST', bool, startup_only=True, section='Message Cache', description='Persist recent message bodies in SQLite so history survives restarts. When disabled, existing message-cache rows are purged at startup and new history remains in RAM only.'),
    'message_cache_respect_no_store': ConfigKeySpec(True, 'MESSAGE_CACHE_RESPECT_NO_STORE', bool, startup_only=True, section='Message Cache', description='Respect XEP-0334 storage hints. no-store messages are excluded from history; no-permanent-store messages remain available only in RAM and are never written to SQLite.'),
    ...
    'translate_from': ConfigKeySpec('auto', 'TRANSLATE_FROM', str, section='Translate', description='Default source language. Use "auto" for provider-side language detection. Set TRANSLATE_TO to a language code such as "de" to allow `,tr` for replies and `,tr text` for direct text. None keeps the target argument mandatory.'),
    'translate_to': ConfigKeySpec(None, 'TRANSLATE_TO', str, section='Translate', description='Default target language. None keeps the target argument mandatory.'),
    'translate_libretranslate_url': ConfigKeySpec('https://translate.envs.net/', 'TRANSLATE_LIBRETRANSLATE_URL', str, allow_empty=True, section='Translate', description='LibreTranslate base URL. The default uses the envs.net LibreTranslate service without an API key. Set an empty string to disable the unauthenticated LibreTranslate fallback.'),
    'translate_libretranslate_languages_url': ConfigKeySpec(None, 'TRANSLATE_LIBRETRANSLATE_LANGUAGES_URL', str, allow_empty=True, section='Translate', description='Optional explicit LibreTranslate /languages endpoint for capability discovery. Leave None/empty to derive it from TRANSLATE_LIBRETRANSLATE_URL. Useful when translation requests use a custom reverse-proxy path.'),
    'translate_libretranslate_api_key': ConfigKeySpec(MISSING, 'TRANSLATE_LIBRETRANSLATE_API_KEY', str, allow_empty=True, section='Translate', description='Optional LibreTranslate API key. Configured API-key providers are tried before the unauthenticated LibreTranslate fallback.', sensitive=True, sample=None),
    'translate_google_api_key': ConfigKeySpec(MISSING, 'TRANSLATE_GOOGLE_API_KEY', str, allow_empty=True, section='Translate', description='Optional Google Cloud Translation Basic v2 API key. Google translation is enabled only when this key is configured; no unauthenticated Google endpoint is used.', sensitive=True, sample=None),
    'translate_deepl_api_key': ConfigKeySpec(MISSING, 'TRANSLATE_DEEPL_API_KEY', str, allow_empty=True, section='Translate', description='Optional DeepL API key. Free keys ending in :fx use api-free.deepl.com; other keys use api.deepl.com.', sensitive=True, sample=None),
    'translate_timeout_seconds': ConfigKeySpec(8, 'TRANSLATE_TIMEOUT_SECONDS', (int, float), section='Translate', description='Translate timeout seconds per provider attempt.'),
