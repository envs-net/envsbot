...
CONFIG_FIELDS: dict[str, ConfigField] = {
    ...
    'database_busy_timeout_ms': ConfigField(5000, 'DATABASE_BUSY_TIMEOUT_MS', int, section='Bot Runtime', description='SQLite runtime tuning. WAL can improve read/write concurrency, but keep it disabled unless your backup/maintenance workflow expects WAL sidecar files.'),
    'database_wal_enabled': ConfigField(False, 'DATABASE_WAL_ENABLED', bool, section='Bot Runtime', description='Database wal enabled.'),
    ...
    'task_stale_after_seconds': ConfigField(3600.0, 'TASK_STALE_AFTER_SECONDS', (int, float), minimum=0, minimum_exclusive=True, section='Bot Runtime', description='Heartbeats older than this are reported by `,tasks stale`.'),
    ...
    'avatar': ConfigField(MISSING, 'AVATAR_PATH', str, section='vCard / Avatar', description='Bot avatar. Set AVATAR_PATH = None to disable avatar publishing. Put your own avatar below data/ and do not overwrite the default repo avatar.', sample='avatar.jpg'),
