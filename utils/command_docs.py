...
def _rss_notes() -> list[str]:
    return [
        ...
        "",
        f"Use `{PREFIX}rss search <query> [page|all|last]` when a long subscription list makes a feed difficult to find. Search is case-insensitive and matches the stable feed number, feed title, subscribed feed URL, and website URL advertised by the feed.",
        "",
        f"Inside a room or MUC PM, search is scoped to that room. In a normal 1:1 chat, trusted users search only their own direct subscriptions while global moderators search all feeds. Add `own`, `rooms`, `mods`, `trusted`, or an explicit room JID before the query to select a narrower permitted scope. Search results intentionally do not expose direct subscriber JIDs.",
        "",
        "```text",
        f"{PREFIX}rss search linux",
        f"{PREFIX}rss search 42",
        f"{PREFIX}rss search own example.org",
        f"{PREFIX}rss search room@conference.example.org kernel 2",
        f"{PREFIX}rss search linux all",
        "```",
        "",
