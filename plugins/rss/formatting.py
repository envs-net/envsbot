...
def _rss_template_usage(bot=None) -> str:
    ...
    return (
        ...
        f"  {prefix}rss template set <feedurl|feed_no> <template>\n"
        f"  {prefix}rss template unset [feedurl|feed_no]\n"
        f"  {prefix}rss template test [feedurl|feed_no] [template]\n"
        "Room or MUC PM (current room is inferred):\n"
        f"  {prefix}rss template [show|set|unset|test] [feedurl|feed_no] [template]\n"
        "Private admin chat for a room:\n"
        f"  {prefix}rss template <show|set|unset|test> <room_jid> "
        "[feedurl|feed_no] [template]\n"
        "Global default (moderator+):\n"
        f"  {prefix}rss template <show|set|unset|test> default [template]\n"
        "Optional: write 'direct' before or after feedurl/feed_no in a 1:1 chat. "
